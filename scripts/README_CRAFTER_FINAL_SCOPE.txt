...
V91 deterministic exact-route verification:
- Production arithmetic and action-state transitions are unchanged from V90.
- V90's width-12000 / 15-second post-solution verification lane was replaced by fixed expanded-state budgets. A 30-second wall-clock limit remains only as an emergency safety valve.
- A compact high-efficiency subset lane (additional only; it never changes the user's enabled skill set) reduces Beam crowding, and a width-4000 all-enabled balanced lane verifies the best action depth.
- Every candidate is re-simulated under the original full user state before comparison.
- Public objective remains: max-quality completion -> fewer actions -> shorter total wait.
- Real-machine regression: Lv100 ★2 Ceviche, Craftsmanship 5778 / Control 5517 / CP633 / 8050 / 17600 / durability70 / initial quality8800, all normal skills enabled -> 17 actions / wait46 / CP29 / durability0.
- The same Ceviche route is required to repeat deterministically in scripts/run-crafter-macro-v91-deterministic-exact-audit.js.
- Existing V90 regressions, V89 exhaustive-optimality cases, V88 full action-state Oracle, best-effort regression, profile audit, boundary audit and fuzz audit must remain passing.
