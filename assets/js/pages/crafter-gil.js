...
function calc(){
 ...
 if(!sell||!materials.length){profitResult.innerHTML='<h3>入力が不足しています</h3><p>完成品の売価と、1つ以上の必要素材を入力してください。</p>';return}
 ...
 profitResult.innerHTML=`<div class="profit-recommend"><strong>比較結果</strong><p>${esc(recommendation)}</p></div><div class="profit-mode-grid">${modeCard(buy,'素材を購入',materials)}${modeCard(self,'素材を自力調達',materials)}</div><div class="tool-actions"><button class="tool-btn" id="addComparison">この結果を比較に追加</button></div>`;
