...
function itemHtml(item){
  const sourceBody=item.sources.length
    ...
    : `<div class="acquisition-none">入手方法を自動取得できませんでした。</div>`;
  const detailUrl=itemDetailUrl(item.id,item.name);
  return `<article class="acquisition-item">
    <div class="acquisition-item-head">
      <div><small>ITEM</small><h3><a class="item-detail-link" href="${detailUrl}" ${detailUrl.startsWith('http')?'target="_blank" rel="noopener"':''}>${FF14Tools.esc(item.name)}</a></h3></div>
      <a class="official-item-link" href="${detailUrl}" ${detailUrl.startsWith('http')?'target="_blank" rel="noopener"':''}>詳細 →</a>
    </div>
    ${sourceBody}
  </article>`;
}
