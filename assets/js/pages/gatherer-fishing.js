...
function officialSearch(name){return 'https://jp.finalfantasyxiv.com/lodestone/playguide/db/search/?q='+encodeURIComponent(name)}
...
function fishHtml(x){
 if(x.error){
   return `<article class="fish-direct-item"><div class="fish-direct-head"><h3>${FF14Tools.esc(x.name)}</h3><a href="${officialSearch(x.name)}" target="_blank" rel="noopener">日本語の公式DBで確認 →</a></div><p>釣り条件を自動取得できませんでした。</p></article>`;
 }
 return `<article class="fish-direct-item">
   <div class="fish-direct-head"><h3>${FF14Tools.esc(x.name)}</h3><a href="${officialSearch(x.name)}" target="_blank" rel="noopener">日本語の公式DBで確認 →</a></div>
...
async function renderFishPage(page=1){
 ...
 fishResult.innerHTML=`<div class="lookup-summary"><strong>「${FF14Tools.esc(fishQueryText)}」：${fishRows.length}件</strong><span>${fishPage} / ${pages} ページ</span></div><div class="lookup-loading">このページの釣り条件を確認しています…</div>`;
 const details=[];
 for(let i=0;i<part.length;i+=4)details.push(...await Promise.all(part.slice(i,i+4).map(fishDetails)));
 fishResult.innerHTML=`<div class="lookup-summary"><strong>「${FF14Tools.esc(fishQueryText)}」：${fishRows.length}件</strong><span>${fishPage} / ${pages} ページ</span></div><div class="fish-direct-list">${details.map(fishHtml).join('')}</div><div class="lookup-pagination">${pageButtons(fishPage,pages)}</div>`;
...
fishSearch.onclick=async()=>{
 ...
 if(!q){fishResult.textContent='魚名を入力してください。';return}
 fishSearch.disabled=true;fishSearch.textContent='検索中…';fishResult.textContent='一致する魚を検索しています…';
 try{
   ...
   fishRows=[];fishResult.innerHTML=`<h3>見つかりませんでした</h3><p>${FF14Tools.esc(e.message||'魚名を短くして検索してください。')}</p><div class="tool-actions"><a class="tool-btn secondary" target="_blank" rel="noopener" href="${officialSearch(q)}">日本語の公式DBで検索</a></div>`
