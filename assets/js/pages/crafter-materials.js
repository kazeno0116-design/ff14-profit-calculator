...
function isCrystal(name){return /(シャード|クリスタル|クラスター)$/.test(name)}
async function expandStructured(itemId,label,qty,depth,maxDepth,base,steps,crystalTotals){
 ...
 for(let i=0;i<ings.length;i++){
   ...
   const childRec=depth+1<maxDepth?await recipeByItem(id):null;step.ingredients.push({name:n,qty:need,crafted:!!childRec});await expandStructured(id,n,need,depth+1,maxDepth,base,steps,crystalTotals);
 }
 steps.push(step);
}
...
async function calculate(c){
 ...
 try{
  const rec=await recipeByItem(c.row_id);if(!rec){recipeSearchResult.innerHTML='<h3>製作レシピが見つかりませんでした</h3><p>別の候補を選ぶか、アイテム名を少し変えて検索してください。</p>';return}
