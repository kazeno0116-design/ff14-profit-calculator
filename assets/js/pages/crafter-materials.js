...
async function calculate(c){
  ...
  try{
    ...
    if(!rec){revealResult(recipeSearchResult);recipeSearchResult.innerHTML='<h3>製作レシピが見つかりませんでした</h3><p>別の候補を選ぶか、アイテム名を少し変えて検索してください。</p>';return}
    const base={},tree=[];
    await expand(c.row_id,name,qty,0,max,base,tree);
    revealResult(recipeSearchResult);recipeSearchResult.innerHTML=`<div class="lookup-summary"><strong>${FF14Tools.esc(name)} × ${qty}</strong><span>必要素材</span></div>`+
  ...
  }catch(e){revealResult(recipeSearchResult);recipeSearchResult.innerHTML='<h3>計算できませんでした</h3><p>データを取得できなかったため、もう一度実行してください。</p>'}
}
