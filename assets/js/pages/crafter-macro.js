...
function renderSkills(){const normal=$('normalSkillList');normal.innerHTML=Object.entries(A).filter(([id])=>id!=='contentAction2').sort((a,b)=>a[1].lv-b[1].lv).map(([id,a])=>`<label class="skill-item"><input class="skill-check" type="checkbox" data-skill="${id}" ${enabledSkills.has(id)?'checked':''}><span>${a.n}</span><span class="skill-meta">Lv${a.lv} / ${skillCategory(a)}</span></label>`).join('');}
function saveSkillSettings(){try{localStorage.setItem(SKILL_STORAGE_KEY,JSON.stringify({enabled:[...enabledSkills]}))}catch(_){}}
function restoreSkillSettings(){try{const raw=localStorage.getItem(SKILL_STORAGE_KEY);if(!raw)return;const d=JSON.parse(raw);if(Array.isArray(d.enabled))enabledSkills=new Set(d.enabled.filter(id=>A[id]&&id!=='contentAction2'));}catch(_){}}
function getSelectedCosmicAction(){return 'none'}
