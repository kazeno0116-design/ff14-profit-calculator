...
document.addEventListener("DOMContentLoaded",()=>{
  ...
  /* V180: laptops up to 1440 CSS px use the same overlay sidebar as mobile so opening
     navigation never subtracts 268px from the article width. */
  const isMobile=()=>matchMedia("(max-width:1440px)").matches;
  ...
  brandToggle?.addEventListener("pointerdown",event=>{
    if(!isMobile())return;
